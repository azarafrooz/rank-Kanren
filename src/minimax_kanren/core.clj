...
(defn sudokufd-with-extra-constraints [puzzle rank]
  (let [vars (repeatedly 81 lvar)
        crank (zipmap vars rank)
        rows (->> vars (partition 9) (map vec) (into []))

        odd-rows (into [] (take-nth 2 rows))
        even-rows (into [] (take-nth 2 (rest rows)))

        odd-rows-odd-indexed (into [] (mapv  #(into [] (take-nth 2 %)) odd-rows))
        odd-rows-even-indexed (into [] (mapv  #(into [] (take-nth 2 (rest %))) odd-rows))
    ...
    (run-crank 10 crank [q]
               ...
               ;(fd/> (first (first rows)) (first (first rows)))
               ;(everyg #(fd/> (first rows) %) (first rows))
               )
...
;        puzzle  (:puzzle options)
;        rank (:rank options)]
;    (time (println (sudokufd-with-extra-constraints puzzle rank)))))